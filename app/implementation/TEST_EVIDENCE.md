...
| 生产 UI 流程 | 14/14 组通过 | [JSON](evidence/ui-browser.json) |
...
PDF 预览在离线状态渲染两页，文档内合成 JavaScript 标记保持未执行，501 页文件降级。HTML/SVG 按文本显示。扩展上下文没有外部网络请求。完整恢复包包含 7 条记录、7 个对象、13 条审计，Python 独立验证全部结构、引用与摘要。
...
GitHub Linux CI 首轮发现保存后刷新可能覆盖新选择，已修复保存期间控件状态和按字段组保存的草稿版本。新增真实 UI 并发修改测试证明旧草稿保留、冲突拒绝、明确载入最新记录，并验证备注与提交状态的待保存编辑可分别提交。
